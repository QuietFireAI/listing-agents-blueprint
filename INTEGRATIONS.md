...
|---|---|---|---|---|---|
| MLS / syndication | OUT+IN | listing publish/status intents | MLS acceptance + live-listing artifact | no | MLS listing ID |
| Lead sources / CRM | IN | lead + inquiry intake intents | n/a (inbound) | no | source lead ID |
| E-signature platform | OUT+IN | agreement/doc signature routing (human signs) | executed-document artifact | no | envelope ID |
| Media vendors (photo/tour) | OUT+IN | vendor order + deliverable intents | media files landed in record | no | order ID |
| Showing service | OUT+IN | showing schedule + feedback intents | confirmed-appointment artifact | no | showing ID |
