...
UNTYPED_WHITELIST = {("11", "routed by content")}
...
def check_regression():
    ...
    if len(skill_files) != 36:
        problems.append(f"expected 36 SKILL.md files, found {len(skill_files)}")
    ...
    for f in ("00-dispatcher/SKILL.md", "07-transaction-coordinator/SKILL.md"):
        m = re.search(r"```json\n(.*?)\n```", open(f).read(), re.S)
        try:
...
def check_meta_layer():
    ...
    for f in ("MANNERS.md", "SWARM.md", "AFTER_ACTION.md", "DISPATCHER_CORE.md",
              "identity/IDENTITY-real-estate-listing-agent.md"):
